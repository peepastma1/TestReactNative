...
export default function Notification({ route }) {
  ...
  async function convertSpeechToText(audioPath) {
    try {
      ...
      responseAnswer = truncateRepeatingCharacters(responseAnswer, 3);
      
      const index = responseAnswer.indexOf(' ', responseAnswer.indexOf(' ') + 1);
        if (index !== -1) {
            responseAnswer = responseAnswer.substring(0, index);
        }
  ...
  function truncateRepeatingCharacters(str, maxRepeats) {
    const regex = new RegExp(`(.)(\\1{${maxRepeats - 1},})`, 'g');
    return str.replace(regex, '$1'.repeat(maxRepeats));
}
